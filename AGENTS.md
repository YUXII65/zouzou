...
- 小程序真机 `wx.request` 必须把 `https://nextstep9.work` 加到微信公众平台的「request 合法域名」；`project.config.json` 的 `urlCheck:false` 只对开发者工具有效，真机仍会拦截。网络失败要显示微信返回的 `errMsg`，不能再落成「用户名或密码不对」。
