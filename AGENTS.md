...
- 关闭「把任务推起来」引导后要直接进入便利贴引导；首组任务完成后不跳转复盘页，而是在书桌显示锚定提示。首组指第一次 AI 计划生成的一批任务（共享 inboxItemId）；必须组内至少完成一件，且没有仍待办/进行中的任务才触发，单条任务完成不触发。引导采用双重判定：前端在最后一件事收尾时触发，服务端在「首组已收尾且尚无复盘」时兜底显示。固定文案：标题「抽屉复盘」，正文「首组任务已完成。点左侧【抽屉】，用复盘留下今天的判断和明天的方向。」
- 复盘生成的下一步任务保留 1-3 条，每条两行：第一行精简标题，第二行具体说明。任务行显示精简标题，展开后显示完整内容；不显示序号或竖线。
- 所有异步按钮和 AI 操作结束后必须可靠清除 loading/pending 状态，不能在响应完成后继续显示「处理中」。`SubmitButton` 在 pending 结束后主动 `router.refresh()`；pending 时保留明确文案和旋转状态，不再显示额外的小刷新按钮。复盘生成和保存 action 完成后重定向回对应日期。
...
- 小程序真机 `wx.request` 只允许已 ICP 备案的 request 合法域名；`nextstep9.work` 未备案，不能直接用于 `wx.request`（`urlCheck:false` 只对开发者工具有效）。未备案时改用微信云开发云函数代理，或把后端绑到一个已备案域名。网络失败要显示微信返回的 `errMsg`，不能再落成「用户名或密码不对」。
- 未备案方案已预留在 `miniprogram/cloudfunctions/apiProxy`：开通微信云开发并创建环境后，把环境 ID 填到 `miniprogram/config.js` 的 `cloudEnv`，所有小程序 API 请求会自动改走 `wx.cloud.callFunction`，云函数再请求 `https://nextstep9.work`。
...
- 首页示例扩充采用「模型产出候选 → 用户 1-10 打分 → 归纳偏好」的迭代流程；不要直接批量填充，也不要只看单条高分就复制其措辞。
- 示例要覆盖具体任务、实用框架、沟通决策、创作学习等不同路数，保持多元；高分样本用于提炼特征，后续继续产出新组合，不机械仿写。
- 示例输入必须语义明确，避免「某个 / 一件事 / 一个方向 / 一个想法」这类没有对象的表达；不为了具体而堆砌数字、日期或虚构细节。
- 示例池不能让简单单步任务占多数；候选项要优先体现走走的强项：多目标取舍、模糊想法澄清、卡点诊断、长期项目重启与持续推进、复盘修正、复杂约束协调。简单任务可以少量保留，但不能代表示例池质量。
- 示例候选要主动跨生活维度轮换：职业 / 项目 / 学习 / 身体 / 金钱 / 居住 / 旅行 / 友谊 / 家庭 / 身份 / 社区 / 数字生活 / 兴趣 / 纪念等不能连续几批集中在同一类。多样性指真实困境与推进方式不同，不是只换名词。
- 后续候选批次按 5-6 个「卡点类型」分组，每类给 2 条，优先覆盖价值冲突、停滞与重启、协作与控制权、注意力与数字生活、照护与责任、金钱与自我价值、结束与告别、复盘与判断、创作与审美、机会成本等方向；不要只按职业或生活题材换皮。
